...
function StaticLighthouse({
  ...
  return (
    <div className="absolute bottom-[4%] left-1/2 top-[55%] w-[72%] -translate-x-1/2" aria-hidden>
      <div className="lighthouse-horizon absolute inset-x-[-38%] bottom-[8%] h-px bg-foreground/10" />
      <div className="lighthouse-tower absolute bottom-0 left-1/2 h-[82%] w-[50%] min-w-12 -translate-x-1/2 [clip-path:polygon(22%_0,78%_0,100%_100%,0_100%)] border-x border-foreground/20 bg-card/90 shadow-[inset_-5px_0_8px_hsl(var(--foreground)/0.04)]">
        {sections.map((index) => (
          <div
            key={index}
      ...
      <div className="absolute bottom-[76%] left-1/2 h-6 w-[62%] -translate-x-1/2 rounded-sm border border-foreground/20 bg-card shadow-[inset_0_-2px_0_hsl(var(--foreground)/0.04)]" />
      <div className="absolute bottom-[86%] left-1/2 h-px w-[78%] -translate-x-1/2 bg-foreground/20">
        <div className="absolute inset-x-[12%] top-1 h-px bg-foreground/10" />
      </div>
      <div className="absolute bottom-[93%] left-1/2 h-0 w-0 -translate-x-1/2 border-x-[1.05rem] border-b-[0.6rem] border-x-transparent border-b-foreground/20" />
      <div className={`lighthouse-lantern absolute bottom-[89%] left-1/2 h-2.5 w-2.5 -translate-x-1/2 rounded-full ${complete ? "bg-amber-300 shadow-[0_0_10px_rgba(245,158,11,0.65)]" : "bg-indigo-500/60"}`} />
      <div className="lighthouse-beam absolute bottom-[88%] left-1/2 h-2 w-36 origin-left -skew-y-[12deg] bg-indigo-400/[0.06]" />
    </div>
  );
}
