...
function StaticLighthouse({
  ...
  return (
    <div className="absolute bottom-[6%] left-1/2 top-[58%] w-[54%] -translate-x-1/2" aria-hidden>
      <div className="lighthouse-horizon absolute inset-x-[-38%] bottom-[8%] h-px bg-foreground/10" />
      <div className="lighthouse-tower absolute bottom-0 left-1/2 h-[76%] w-[43%] min-w-10 -translate-x-1/2 [clip-path:polygon(22%_0,78%_0,100%_100%,0_100%)] border-x border-foreground/20 bg-card/90 shadow-[inset_-5px_0_8px_hsl(var(--foreground)/0.04)]">
        {sections.map((index) => (
          <div
            key={index}
      ...
      <div className="absolute bottom-[72%] left-1/2 h-5 w-[52%] -translate-x-1/2 rounded-sm border border-foreground/20 bg-card shadow-[inset_0_-2px_0_hsl(var(--foreground)/0.04)]" />
      <div className="absolute bottom-[81%] left-1/2 h-px w-[70%] -translate-x-1/2 bg-foreground/20">
        <div className="absolute inset-x-[12%] top-1 h-px bg-foreground/10" />
      </div>
      <div className="absolute bottom-[88%] left-1/2 h-0 w-0 -translate-x-1/2 border-x-[0.9rem] border-b-[0.5rem] border-x-transparent border-b-foreground/20" />
      <div className={`lighthouse-lantern absolute bottom-[84%] left-1/2 h-2 w-2 -translate-x-1/2 rounded-full ${complete ? "bg-amber-300 shadow-[0_0_9px_rgba(245,158,11,0.65)]" : "bg-indigo-500/60"}`} />
      <div className="lighthouse-beam absolute bottom-[83%] left-1/2 h-2 w-28 origin-left -skew-y-[12deg] bg-indigo-400/[0.06]" />
    </div>
  );
}
