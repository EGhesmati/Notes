...
function StaticLighthouse({
  ...
  return (
    <div className="absolute bottom-[12%] left-1/2 top-[52%] w-[46%] -translate-x-1/2" aria-hidden>
      <div className="absolute inset-x-[-38%] bottom-[8%] h-px bg-foreground/10" />
      <div className="absolute bottom-0 left-1/2 h-[72%] w-[38%] min-w-9 -translate-x-1/2 [clip-path:polygon(22%_0,78%_0,100%_100%,0_100%)] border-x border-foreground/20 bg-card/90 shadow-[inset_-5px_0_8px_hsl(var(--foreground)/0.04)]">
        {sections.map((index) => (
          <div
            key={index}
      ...
      <div className="absolute bottom-0 left-1/2 h-2 w-[58%] -translate-x-1/2 rounded-sm border border-foreground/15 bg-foreground/[0.07]" />
      ...
      <div className={`absolute bottom-[80%] left-1/2 h-1.5 w-1.5 -translate-x-1/2 rounded-full ${complete ? "bg-amber-300 shadow-[0_0_8px_rgba(245,158,11,0.65)]" : "bg-indigo-500/60"}`} />
      <div className="absolute bottom-[72%] left-[57%] h-10 w-24 origin-left -skew-y-[18deg] bg-indigo-400/[0.07]" />
    </div>
  );
}
