...
export function PomodoroStatsPanel() {
  ...
  return (
    <div className="relative">
      ...
      {open && stats && (
        <div className="absolute right-0 top-full mt-2 w-80 max-h-[70vh] overflow-y-auto rounded-xl border border-border bg-card/95 p-1 shadow-xl backdrop-blur-xl">
          ...
          <div className="border-t border-border/50">
            <div className="p-3">
              <p className="mb-2 text-[10px] font-semibold uppercase tracking-wider text-muted-foreground/60">Today</p>
            ...
            <div className="border-t border-border/30 p-3">
              <p className="mb-2 text-[10px] font-semibold uppercase tracking-wider text-muted-foreground/60">This Week</p>
              <div className="flex items-center justify-between text-xs">
                <span className="flex items-center gap-1 text-muted-foreground">
            ...
            <div className="border-t border-border/30 p-3">
              <p className="mb-2 text-[10px] font-semibold uppercase tracking-wider text-muted-foreground/60">This Month</p>
              <div className="flex items-center justify-between text-xs">
                <span className="flex items-center gap-1 text-muted-foreground">
              ...
              {stats.month.bestDay && (
                <p className="mt-1.5 text-[10px] text-muted-foreground/60">
                  <Trophy className="mr-0.5 inline h-3 w-3 text-amber-400/60" />
            ...
            <div className="border-t border-border/30 p-3">
              <p className="mb-2 text-[10px] font-semibold uppercase tracking-wider text-muted-foreground/60">Last 7 Days</p>
                ...
                {stats.dailyChart.map((d) => {
                  ...
                  return (
                    <div key={d.label} className="flex items-center gap-2 text-[10px]">
                      <span className="w-8 text-right text-muted-foreground/60">{d.label}</span>
                      <div className="flex-1 rounded-full bg-muted/60">
                        <div
                          className="h-3 rounded-full bg-primary/40 transition-all"
                          style={{ width: `${(width / maxBar) * 100}%` }}
                        />
                      </div>
                      <span className="w-6 text-left font-mono text-muted-foreground/60">{d.sessions}</span>
                    </div>
                  );
                })}
              </div>
            ...
            {(stats.currentStreak > 0 || stats.longestStreak > 0) && (
              <div className="border-t border-border/30 p-3">
                <p className="mb-2 text-[10px] font-semibold uppercase tracking-wider text-muted-foreground/60">Streaks</p>
            ...
            {stats.noteBreakdown.length > 0 && (
              <div className="border-t border-border/30 p-3">
                <p className="mb-2 text-[10px] font-semibold uppercase tracking-wider text-muted-foreground/60">By Note</p>
                <div className="space-y-1.5">
                  {stats.noteBreakdown.map((n) => (
                    <div key={n.noteId} className="flex items-center gap-2 text-[10px]">
                      <StickyNote className="h-3 w-3 shrink-0 text-muted-foreground/50" />
                      <span className="flex-1 truncate text-foreground/80">{n.noteTitle}</span>
            ...
            <div className="border-t border-border/30 p-3">
              <p className="mb-2 text-[10px] font-semibold uppercase tracking-wider text-muted-foreground/60">Completion</p>
              <div className="flex items-center gap-2">
                <div className="flex-1 rounded-full bg-muted/60">
                  <div
                    className="h-3 rounded-full bg-emerald-500/60 transition-all"
                    style={{ width: `${stats.completionRate}%` }}
                  />
                </div>
                <span className="text-[10px] font-mono font-medium text-muted-foreground/60">{stats.completionRate}%</span>
              </div>
            </div>
          </div>
        </div>
